...
public class UserService {
    ...
    // login for visitor
    public void visitorLog(User[] users) throws ValidationLoginException, StringReadException{
        ...
        while (!logged) {
            try {
                ...
                if (userDao.roleVisitor(visitorName)) {
                    System.out.println("You have been successfully logged in as a Visitor.");
                    logged = true; 
                } else {
                    throw new ValidationLoginException("You failed to log in as a Visitor.");
                }
            } catch (StringReadException e) {
                System.out.println("Error: " + e.getMessage());
            } catch (ValidationLoginException e) {
                System.out.println("Error: " + e.getMessage());
            }
        }
        }
    ...
    //login for client
    public void clientLog(User[] users) throws ValidationLoginException, StringReadException{
        boolean logged = false;
        ...
        while (!logged) {
            try {
                ...
                if (userDao.search(username, password) && userDao.roleClient(username)) {
    ...
    public void add() throws StringReadException, ArrayFullException{
        ...
        if (userDao.getUsers().length >= MAX_USERS) {
            throw new ArrayFullException("User array is full. Cannot add more tasks.");
        }
    ...
    public void register(User[] users) throws StringReadException{
        ...
        while (!registered) {
            try {
                ...
                if (firstnm.isEmpty() || lastnm.isEmpty() || usernm.isEmpty() || pass.isEmpty()) {
                    ...
                    // Simulating registration completion
